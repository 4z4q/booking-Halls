  ...
  const handleSortChange = (value: string) => {
    ...
    switch (value) {
      case "newest":
        ...
        sortedReviews.sort((a, b) => new Date(b.user.date).getTime() - new Date(a.user.date).getTime())
        break
      case "oldest":
        // Sort by date (oldest first)
        sortedReviews.sort((a, b) => new Date(a.user.date).getTime() - new Date(b.user.date).getTime())
        break
      case "highest":
          ...
          <div key={review.id} className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <Button
                variant="ghost"
                size="sm"
                className={`text-sm ${helpfulClicked[review.id] ? "text-purple-600" : ""}`}
